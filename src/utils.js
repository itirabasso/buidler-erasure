...
export const hexlify = utf8str => utils.hexlify(utils.toUtf8Bytes(utf8str));

const createPaddedMultihashSha256 = string => {
  const hash = utils.sha256(utils.toUtf8Bytes(string));
  const sha2_256 = utils.hexZeroPad("0x12", 8); // uint8
  const bits256 = utils.hexZeroPad(utils.hexlify(64), 8);
...
export const createMultihashSha256 = string => {
  const hash = utils.sha256(utils.toUtf8Bytes(string));
...
function createInstanceAddressWithCallData(
  ...
  const initCodeHash = utils.solidityKeccak256(
    ["bytes", "bytes"],
    [spawnBytecode, initCallData]
  );

  if (!salt) {
    salt = utils.solidityKeccak256(["address", "uint256"], [sender, nonce]);
  }

  const create2hash = utils.solidityKeccak256(
    ["bytes1", "address", "bytes32", "bytes32"],
    ["0xff", factoryContractAddress, salt, initCodeHash]
  );

  let instanceAddress = utils.getAddress(
